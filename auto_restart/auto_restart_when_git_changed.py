...
def restart(args=None):
    os.system('git pull')
    kill_str = f'''ps -aux|grep {script_name} |grep -v grep|grep -v auto_restart_when_git_changed |awk '{{print $2}}' |xargs kill -9;'''
    # kill_str = f'''ps auxww | grep '{script_name} ' | awk '{{print $2}}' | xargs kill'''
